...
double der_f( double x, double f, double g ) { return g; }
double der_g( double x, double f, double g )
{
    return pow(sqrt(f), 3) / sqrt(x);
}

vd calculator( vd X, double f0, double g0 )
{
    vd F;
...
int main(int argc, const char *argv[])
{
    ...
    while ((eps_right - eps_left) >= 1e-10)
    {
        ...
        F = calculator(X, y0, der_y0 - eps);
        if (F.size() == X.size() || F[F.size()-1] > 1) {eps_left = eps;}
        else { eps_right = eps; }
    }
