...
vp rk4( vd X, double f0, double g0 )
{
    vp F;
...
int main(int argc, const char *argv[])
{
    ...
    **************************************************************************/
    ...
    while ((eps_right - eps_left) >= 1e-10)
    {
        ...
        if (F.size() == X.size() || F[F.size()-1].first > 1)
            eps_left = eps;
        else
            eps_right = eps;
    }
    ...
    FILE *file;
    size_t size = 1000;
    size_t step = len / size + ((len % size) ? 1 : 0);
    file = fopen("data.py", "w");
    fprintf(file, "x0 = %f; y0 = %f; der_y0 = %f\n", x0, y0, der_y0 - eps);
    fprintf(file, "X = [ ");
    forn(i, size)
        fprintf(file, "%f, ", X[i * step]);
    fprintf(file, "]\n");
    fprintf(file, "F = [ ");
    forn(i, size)
        fprintf(file, "%f, ", F[i * step].first);
    fprintf(file, "]\n");
    fprintf(file, "F1 = [ ");
    forn(i, size)
        fprintf(file, "%f, ", F[i * step].second);
    fprintf(file, "]\n");
    fclose(file);
    return 0;
}
